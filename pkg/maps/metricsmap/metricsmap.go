...
// getNumCPUs returns a total number of possible CPUS, i.e. CPUs that have been
// allocated resources and can be brought online if they are present, by reading
// "/sys/device/system/cpu/possible".
//
// TODO comment about the format and PERCPU allocs
func getNumCPUs() int {
	return getNumCPUsFromFile("/sys/devices/system/cpu/possible")
}

func getNumCPUsFromFile(path string) int {
	...
	for _, s := range strings.Split(string(out), ",") {
		var start, end int
		// Go's scanf returns an error if a format cannot be fully matched, so
		// just ignore it
